  ...
  if (!schemaPromise) {
    schemaPromise = (async () => {
      await sql`CREATE TABLE IF NOT EXISTS supplements (
        ...
        content_unit TEXT NOT NULL CHECK (content_unit IN ('เม็ด','ช้อน','ซอง')), is_active BOOLEAN NOT NULL DEFAULT TRUE,
      ...
      await sql`CREATE TABLE IF NOT EXISTS supplement_courses (
        ...
        total NUMERIC(12,2) NOT NULL, created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
      )`;
