...
const recommendFoodNutrition = async (nutrientFocus: string, timeOfDay: string): Promise<FoodRecommendationResult | null> => {
  ...
  if (timeOfDay === 'มื้อเช้า') {
    specialInstruction = `\n\n**ข้อบังคับ:** สำหรับเมนูที่ 3 ให้ตั้งชื่อเมนู (name) โดยขึ้นต้นด้วย "${breakfastBaseName}" แล้วตามด้วยคำแนะนำการชงกับน้ำเย็นด้วยภาษาที่หลากหลายไม่ซ้ำกัน (ห้ามระบุปริมาณน้ำ เช่น 250ml) (พลังงาน 300 kcal, P 33g, C 23g, F 6.5g, Fiber 5g) เสมอ และให้คุณแต่งประโยคอธิบายเหตุผล (reason) ให้แตกต่างกันไปในแต่ละครั้ง โดยอ้างอิงความโดดเด่นของสารอาหารเหล่านี้`;
  } else if (nutrientFocus === 'โปรตีน') {
    specialInstruction = '\n\n**ข้อบังคับ:** สำหรับเมนูที่ 3 ให้ตั้งชื่อเมนู (name) โดยขึ้นต้นด้วย "นิวทริไลท์ ออลแพลนท์ โปรตีน 1 ช้อนเขียว" แล้วตามด้วยคำแนะนำการชงกับน้ำเย็นด้วยภาษาที่หลากหลายไม่ซ้ำกัน (ห้ามระบุปริมาณน้ำ เช่น 250ml) (พลังงาน 80 kcal, P 17g, C 1g, F 0g) เสมอ และให้คุณแต่งประโยคอธิบายเหตุผล (reason) ให้แตกต่างกันไปในแต่ละครั้ง โดยอ้างอิงความโดดเด่นของโปรตีนเพียวที่ไม่มีไขมันและน้ำตาล';
  }
