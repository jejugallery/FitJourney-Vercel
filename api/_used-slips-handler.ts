...
export default async function usedSlipsHandler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      if (!slipId || typeof slipId !== 'string') return res.status(400).json({ error: 'slipId query parameter is required' });
    ...
    if (req.method === 'POST') {
      ...
      if (!bodySlipId || !billingId || !userId) return res.status(400).json({ error: 'slipId, billingId, and userId are required' });
      const result = await sql`
        ...
        ON CONFLICT (slip_id) DO NOTHING RETURNING *
      `;
