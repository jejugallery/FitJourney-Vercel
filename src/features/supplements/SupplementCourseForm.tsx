...
import { CASHBACK_PERCENTAGES, calculateCashback, calculateCourseLine, calculateCourseTotals } from './pricing';
...
export default function SupplementCourseForm({ trainees, supplements, onSaved }: Props) {
  ...
  const cashbackAmount = useMemo(() => calculateCashback(totals.total, cashbackPercent), [totals.total, cashbackPercent]);
  ...
  if (pickerOpen) return <section className="supplement-screen">
    ...
    {!availableCount ? <div className="supplement-state">ยังไม่มีอาหารเสริมในคลัง</div> : !available.length ? <div className="supplement-state">ไม่พบอาหารเสริม</div> : <div className="supplement-picker-grid">{available.map(item => <button type="button" key={item.id} className="supplement-product-card" onClick={() => add(item)}><img src={item.imageUrl} alt="" /><span className="supplement-card-copy"><b>{item.name}</b><small>{item.contentQuantity} {item.contentUnit}</small><strong>฿{money(item.price)}</strong></span><span className="supplement-card-add">＋</span></button>)}</div>}
  </section>;
  ...
  return <div className="supplement-course-form">
    ...
    <div className="supplement-course-list">{lines.map(line => { const price = calculateCourseLine(line.supplement.price, line.packageQuantity, line.discountType, line.discountValue); return <article key={line.lineId} className="supplement-course-card">
      <div className="supplement-card-head"><img src={line.supplement.imageUrl} alt="" /><div className="supplement-card-copy"><b>{line.supplement.name}</b><small>{line.supplement.contentQuantity} {line.supplement.contentUnit}</small><span>฿{money(line.supplement.price)} / ชิ้น</span></div><button type="button" onClick={() => setLines(current => current.filter(item => item.lineId !== line.lineId))} className="supplement-icon-danger" aria-label={`ลบ ${line.supplement.name}`}>✕</button></div>
    ...
    {!!lines.length && <div className="supplement-total"><div><span>ยอดก่อนส่วนลด</span><b>฿{money(totals.subtotal)}</b></div><div><span>ส่วนลดรวม</span><b className="supplement-discount-text">-฿{money(totals.discountTotal)}</b></div><div className="grand"><span>รวมสุทธิ</span><b>฿{money(totals.total)}</b></div><label className="supplement-cashback-control"><span>เปอร์เซ็นต์ได้เงินคืน</span><select className="supplement-field" value={cashbackPercent} onChange={e => setCashbackPercent(Number(e.target.value))}>{CASHBACK_PERCENTAGES.map(percent => <option key={percent} value={percent}>{percent}%</option>)}</select></label><div className="supplement-cashback-row"><span>ได้เงินคืนภายหลัง ({cashbackPercent}%)</span><b>฿{money(cashbackAmount)}</b></div><button type="button" className="supplement-action supplement-action-primary" onClick={save} disabled={saving}>{saving ? 'กำลังบันทึก...' : 'บันทึกและดาวน์โหลด PDF'}</button></div>}
  </div>;
}
