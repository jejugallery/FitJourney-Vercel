...
export default function SupplementCourseHistory({ trainees, refreshKey, normalizeCourse }: Props) {
  ...
  if (active) return <section className="supplement-screen">
    ...
    <div className="supplement-total"><div><span>ยอดก่อนส่วนลด</span><b>฿{money(active.subtotal)}</b></div><div><span>ส่วนลดรวม</span><b className="supplement-discount-text">-฿{money(active.discountTotal)}</b></div><div className="grand"><span>รวมสุทธิ</span><b>฿{money(active.total)}</b></div><button className="supplement-action supplement-action-primary" onClick={() => downloadSupplementCoursePdf(active).catch(() => alert('สร้าง PDF ไม่สำเร็จ กรุณาลองใหม่'))}>ดาวน์โหลด PDF</button></div>
  </section>;
