...
export default function SupplementCourseHistory({ trainees, refreshKey, normalizeCourse, currentTrainerId }: Props) {
  ...
  if (active) return <section className="supplement-screen">
    ...
    <div className="supplement-snapshot-list">{orderSupplementProducts(active.items, item => item.supplementName, item => item.unitPrice).map(item => { const grossAmount = Number(item.grossAmount ?? (item.unitPrice * item.packageQuantity)); const qty = Math.max(1, Math.trunc(Number(item.packageQuantity) || 1)); return <article key={item.id} className="supplement-snapshot-card"><div className="supplement-snapshot-images">{Array.from({ length: qty }).map((_, idx) => (item.imageUrl ? <img key={idx} src={item.imageUrl} alt="" /> : <div key={idx} className="supplement-snapshot-noimg">📦</div>))}</div><div className="supplement-card-copy"><b>{item.supplementName}</b><small>{item.contentQuantity} {item.contentUnit}</small><span>{formatCourseItemUnitPrice(item.unitPrice)} <strong className="supplement-item-qty">× {item.packageQuantity}</strong></span></div><div className="supplement-snapshot-price">{Number(item.discountAmount || 0) > 0 && <span className="supplement-item-gross">฿{money(grossAmount)}</span>}{Number(item.discountAmount || 0) > 0 && <span className="supplement-item-discount">ส่วนลด -฿{money(item.discountAmount)}</span>}<strong>฿{money(item.netAmount)}</strong></div></article>; })}</div>
    <div className="supplement-total"><div><span>ยอดก่อนส่วนลด</span><b>฿{money(active.subtotal)}</b></div>{Number(active.discountTotal || 0) > 0 && <div><span>ส่วนลดรวม</span><b className="supplement-discount-text">-฿{money(active.discountTotal)}</b></div>}<div className="grand"><span>รวมสุทธิ</span><b>฿{money(active.total)}</b></div>{Number(active.cashbackAmount || 0) > 0 && <div className="supplement-cashback-row"><span>ได้เงินคืนภายหลัง ({Number(active.cashbackPercent)}%)</span><b>฿{money(active.cashbackAmount)}</b></div>}<button className="supplement-action supplement-action-primary" style={{ width: '100%', backgroundColor: '#3b82f6', color: 'white', marginBottom: '12px' }} onClick={openDashboard} disabled={openingDashboard}>{openingDashboard ? 'กำลังเปิด...' : 'บันทึกเป็นรูปภาพ'}</button>{active.trainerId === currentTrainerId && <button type="button" className="supplement-action supplement-action-danger" onClick={remove} disabled={deleting}>{deleting ? 'กำลังลบ...' : 'ลบประวัติคอร์ส'}</button>}</div>
  </section>;
