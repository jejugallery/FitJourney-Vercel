...
export default function SupplementCourseHistory({ trainees, refreshKey, normalizeCourse }: Props) {
  ...
  if (active) return <section className="supplement-screen">
    ...
    <div className="supplement-total"><div><span>ยอดก่อนส่วนลด</span><b>฿{money(active.subtotal)}</b></div><div><span>ส่วนลดรวม</span><b className="supplement-discount-text">-฿{money(active.discountTotal)}</b></div><div className="grand"><span>รวมสุทธิ</span><b>฿{money(active.total)}</b></div>{Number(active.cashbackPercent || 0) > 0 && <div className="supplement-cashback-row"><span>ได้เงินคืนภายหลัง ({Number(active.cashbackPercent)}%)</span><b>฿{money(active.cashbackAmount)}</b></div>}<button className="supplement-action supplement-action-primary" onClick={() => downloadSupplementCoursePdf(active).catch(() => alert('สร้าง PDF ไม่สำเร็จ กรุณาลองใหม่'))}>ดาวน์โหลด PDF</button></div>
  </section>;
  ...
  return <div>
    ...
    {loading ? <div className="supplement-state">กำลังโหลด...</div> : !courses.length ? <div className="supplement-state">ยังไม่มีประวัติคอร์ส</div> : <div className="supplement-history-list">{courses.map(course => <article key={course.id} className="supplement-history-card"><div className="supplement-history-top"><div><small>ลูกเทรน</small><b>{course.traineeName}</b></div><strong>฿{money(course.total)}</strong></div><div className="supplement-history-meta"><span>📅 {new Date(course.createdAt).toLocaleDateString('th-TH')}</span><span>📦 {course.itemCount} รายการ</span>{Number(course.cashbackPercent || 0) > 0 && <span>💰 คืน {Number(course.cashbackPercent)}% · ฿{money(course.cashbackAmount)}</span>}</div><button type="button" className="supplement-action supplement-action-secondary" onClick={() => open(course.id)} disabled={opening}>{opening ? 'กำลังเปิด...' : 'ดูรายละเอียด'}</button></article>)}</div>}
  </div>;
}
