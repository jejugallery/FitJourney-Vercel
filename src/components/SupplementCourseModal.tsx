...
interface Props { onClose: () => void; trainees: CourseTrainee[]; isSuperadmin: boolean; currentTrainerId: string; }
...
export default function SupplementCourseModal({ onClose, trainees, isSuperadmin, currentTrainerId }: Props) {
  const [tab, setTab] = useState<Tab>('course');
  const [editCourse, setEditCourse] = useState<SavedSupplementCourse | null>(null);
  ...
  const saved = async () => { setHistoryKey(key => key + 1); setEditCourse(null); };

  const handleEdit = (course: SavedSupplementCourse) => {
    setEditCourse(course);
    setTab('course');
  };

  return <div className="supplement-modal-backdrop" onClick={onClose}><div className="supplement-modal" onClick={event => event.stopPropagation()}><button type="button" className="supplement-close" onClick={onClose}>&times;</button><h2 style={{ margin: '0 45px 14px 0' }}>จัดคอร์สอาหารเสริม</h2>
    ...
    {loading ? <p style={{ textAlign: 'center', padding: 30 }}>กำลังโหลด...</p> : error ? <div style={{ textAlign: 'center', padding: 30, color: '#dc2626' }}>{error}<br /><button onClick={load} style={{ marginTop: 10 }}>ลองใหม่</button></div> : tab === 'course' ? <SupplementCourseForm trainees={trainees} supplements={supplements.filter(item => item.isActive)} onSaved={saved} initialCourse={editCourse} /> : tab === 'history' ? <SupplementCourseHistory trainees={trainees} refreshKey={historyKey} normalizeCourse={normalizeCourse} currentTrainerId={currentTrainerId} onEdit={handleEdit} /> : <SupplementCatalogPanel supplements={supplements} onRefresh={load} />}
  </div></div>;
}
