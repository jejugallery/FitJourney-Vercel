...
export default function FoodHistoryModal({ targetId, onClose }: FoodHistoryModalProps) {
  ...
  useEffect(() => {
    ...
    const fetchGoal = async () => {
      try {
        const q = query(collection(db, 'recommendation'), where('targetId', '==', targetId));
    ...
    const checkRole = async () => {
      ...
      try {
        ...
        if (!adminSnap.empty) {
          ...
          if (status === 'อนุมัติ' || status === 'superadmin') setIsTrainer(true);
        }
      } catch (err) {
        console.error(err);
      }
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      ...
      setLoading(true);
      try {
        const q = query(collection(db, 'foodLogs'), where('traineeId', '==', targetId));
        const snap = await getDocs(q);
        const data = snap.docs.map(d => ({ id: d.id, ...d.data() } as FoodLog));
        setAllLogs(data);
      } catch (err) {
  ...
  const handleShareLogToLine = async (log: FoodLog) => {
    try {
      ...
      const flexMessage = {
        type: "flex", altText: "เทรนเนอร์ตรวจอาหารของคุณแล้ว",
        contents: {
          type: "bubble",
          hero: { type: "image", url: log.imageUrl || "https://firebasestorage.googleapis.com/v0/b/fitjourneythailand.appspot.com/o/default-food.png?alt=media", size: "full", aspectRatio: "20:13", aspectMode: "cover" },
          body: {
            type: "box", layout: "vertical",
            contents: [
              { type: "text", text: "ผลตรวจอาหาร", weight: "bold", size: "xl", color: "#1DB446" },
              { type: "text", text: `ของ ${traineeName || 'ลูกเทรน'}`, size: "xs", color: "#94a3b8", margin: "xs" },
          ...
          footer: { type: "box", layout: "vertical", spacing: 'sm', contents: [{ type: "button", style: "primary", color: "#ef4444", height: "sm", action: { type: "uri", label: "ส่งอาหารเพิ่ม", uri: "https://liff.line.me/2010284484-jvUDlx0u?action=upload-food" } }], flex: 0 }
        }
      };
    ...
    <>
      ...
      >
        ...
        >
          ...
          {traineeImage ? (
            <img src={traineeImage} alt={traineeName} style={{ width: '38px', height: '38px', borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
          ) : (
            <div style={{ width: '38px', height: '38px', borderRadius: '50%', background: '#cbd5e1', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '1.2rem', flexShrink: 0 }}>👤</div>
          )}
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, fontSize: '1.1rem', color: 'var(--text-main)' }}>ประวัติอาหารทั้งหมด</div>
            {traineeName && <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{traineeName}</div>}
          </div>
          <button
            onClick={onClose}
            style={{
        ...
        <div style={{ padding: '1rem 1.25rem', background: '#fff', borderBottom: '1px solid #e2e8f0' }}>
            ...
            {[3, 7, 14, 30].map(d => (
              <button
                ...
                style={{
                  ...
                  background: daysBack === d && !dateFrom && !dateTo ? 'var(--primary)' : '#f1f5f9',
          ...
          {/* Date range filter */}
          <div style={{ background: '#f8fafc', borderRadius: '10px', border: '1px solid #e2e8f0', padding: '0.65rem 0.85rem' }}>
            <div style={{ fontSize: '0.8rem', fontWeight: 'bold', color: '#475569', marginBottom: '0.5rem' }}>หรือเลือกช่วงวันที่:</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              {(dateFrom || dateTo) && (
                <button
                  ...
                  style={{
                    alignSelf: 'flex-end', background: '#fef2f2',
        ...
        <div style={{ padding: '1.25rem', flex: 1 }}>
          {loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
              {dateKeys.map(date => (
                <div key={date}>
                  <h3 style={{ fontSize: '1.1rem', color: '#334155', marginBottom: '1rem', paddingBottom: '0.5rem', borderBottom: '2px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    📅 {date}
                  </h3>
                    ...
                    {groupedLogs[date].map(log => (
                      <div key={log.id} style={{ background: '#fff', borderRadius: '16px', overflow: 'hidden', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
                        <div style={{ position: 'relative', width: '100%', height: '200px' }}>
                          ...
                          {traineeGoal && (
                            <div style={{
                              ...
                              fontSize: '0.75rem', fontWeight: 'bold', pointerEvents: 'none', zIndex: 2,
                        ...
                        <div style={{ padding: '1.25rem' }}>
                          ...
                          {log.details && (
                            <div style={{ background: '#f1f5f9', padding: '0.75rem 1rem', borderRadius: '8px', marginBottom: '1rem', fontSize: '0.9rem', color: '#334155', borderLeft: '4px solid #94a3b8' }}>
                            ...
                            <div style={{ background: '#f0fdf4', borderLeft: '4px solid #22c55e', padding: '1rem', borderRadius: '4px' }}>
                              ...
                              {isTrainer && (
                                <button
                                  ...
                                  style={{ marginTop: '10px', width: '100%', padding: '8px', background: '#06C755', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold', fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', boxShadow: '0 2px 4px rgba(6,199,85,0.2)' }}
                                >
      ...
      {fullscreenImage && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.9)', zIndex: 160000, display: 'flex', justifyContent: 'center', alignItems: 'center', touchAction: 'none' }}>
          ...
          <TransformWrapper initialScale={1} minScale={1} maxScale={5} centerOnInit={true} centerZoomedOut={true}>
            <TransformComponent wrapperStyle={{ width: '100vw', height: '100vh' }} contentStyle={{ width: '100vw', height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
