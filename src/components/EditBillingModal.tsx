...
export default function EditBillingModal({ billing, onClose }: EditBillingModalProps) {
  ...
  const [accountName, setAccountName] = useState(billing.accountName || '');
  const [accountNumber, setAccountNumber] = useState(billing.accountNumber || '');
  const [description, setDescription] = useState(billing.description || '');
  const [invitationText, setInvitationText] = useState(billing.invitationText || 'อย่าลืมโอนกันนะ 💸');
  const [invitationColor, setInvitationColor] = useState(billing.invitationColor || '#ef4444');
  const [buttonColor, setButtonColor] = useState(billing.buttonColor || '#6d28d9');
  ...
  const userId = billing.createdBy;
  ...
  const handleDeleteSavedAccount = async () => {
    ...
    if (!account || !userId || !confirm(`ลบบัญชี “${account.accountName}” ออกจากบัญชีที่บันทึกไว้?`)) return;
          ...
          <div style={{
            ...
            <div>
              ...
              <input
                ...
                placeholder="เช่น อย่าลืมโอนกันนะ 💸"
              />
