...
export default function CreateEventModal({ onClose, userId }: CreateEventModalProps) {
  ...
  const [invitationColor, setInvitationColor] = useState('#6d28d9'); // default purple
  const [linkType, setLinkType] = useState('none'); // 'none' | 'zoom' | 'register' | 'details' | 'custom'
  const [linkLabel, setLinkLabel] = useState('');
  const [linkUrl, setLinkUrl] = useState('');
  const [buttonColor, setButtonColor] = useState('#ef4444'); // default red
    ...
    <div style={{
      ...
      <div style={{
        ...
        <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          ...
          >
            {saving ? 'กำลังสร้าง...' : 'สร้างกิจกรรม'}
          </button>
        </form>
      </div>
    </div>
